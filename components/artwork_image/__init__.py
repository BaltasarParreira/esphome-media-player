...
async def to_code(config):
    ...
    # =========================================================================
    # REAL-TIME ASYNC CMAKE PATCHER (RUNS IN BACKGROUND DURING GENERATION)
    # =========================================================================
    from esphome.core import CORE
    if CORE.is_esp32 and CORE.using_esp_idf:
        import threading
        ...
        # 1. Register your bundled component path natively to the CMake index
        esp32.add_idf_component(
            name="libjpeg-turbo-esp32",
            path=os.path.join(os.path.dirname(__file__), "..", "libjpeg-turbo-esp32")
        )

        # 2. Define the background file monitor and patcher
        def cmake_monitor_worker():
            target_path = os.path.join(CORE.build_dir, "src", "CMakeLists.txt")
            
            # Poll for up to 10 seconds waiting for ESPHome to dump the file to disk
            for _ in range(100):
                if os.path.exists(target_path):
                    try:
                        with open(target_path, "r", encoding="utf-8") as f:
                        ...
                        # Check if the requirement is already there to avoid looping
                        if "idf_component_register" in content and "libjpeg-turbo-esp32" not in content:
                            patched = content.replace(
                                "idf_component_register(", 
                                "idf_component_register(\n    REQUIRES libjpeg-turbo-esp32\n"
                            ...
                            with open(target_path, "w", encoding="utf-8") as f:
                                f.write(patched)
                            break # Successfully patched, exit thread
                    except Exception:
                        pass # File might be locked momentarily, retry
                time.sleep(0.1)

        # 3. Fire the thread immediately into the background
        threading.Thread(target=cmake_monitor_worker, daemon=True).start()
